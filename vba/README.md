...
- `Mapping.bas` … **「どのカラムをどのセルにどう見せるか」「どのボタンにどのマクロを割り当てるか」
  ...
  割り付け・表示だけを表で差し替えられる。
- `Console.bas` … **受付 → 情報確認 → 掲示板 → CE 指示（ディスパッチ）** をタブ切り替えで一連に回せる
  オペレータ用コンソールを、**疲れにくいダークモード**で構築（エントリポイント `BuildConsole`）。

> バックエンドDBについて: 作業履歴などの実体は社内サーバーの **ACROS EUC** データベースを想定しています
> （現行 VBA には未記載）。取得は `CEInstruction.bas` の作業履歴設定（`WH_*`）で ODBC 接続先・表名・列名を
> 指定してください。DSN 文字列が分かり次第、プレースホルダを置き換えれば動作します。
...
## Console.bas（受付→CEディスパッチ コンソール）

オペレータが一連の流れをタブで切り替えながら操作できるコンソールを構築します。

| プロシージャ | 役割 |
| --- | --- |
| `BuildConsole` | 「コンソール」ホーム画面＋各タブへのジャンプ＋各シートの「戻る」ナビ＋ステップボタンを構築（初回1回）。 |
| `Console_Receive` | ①受付：コンソールの CASE_ID を「メール作成」`D4` に渡して `GetCaseData` を実行。 |
| `Console_RunAll` | ①→④を一括実行（受付→作業履歴→掲示板→CE指示メール）。 |
| `ApplyDarkTheme` | 既存の各シートにもダークテーマを適用。 |
| `GotoSheet` | タブ（各シート）へジャンプ（ナビボタンから呼ばれる）。 |

### ダークモード配色（眼精疲労を抑える設計）

純黒×純白の高コントラストを避け、暖色寄りの低刺激な配色にしています。Excel のフォームボタンは
配色できないため、**角丸シェイプをボタン化**して色を反映します。

| 用途 | 色 | ねらい |
| --- | --- | --- |
| 背景 | `#1B1D1E` | 純黒を避けた濃いグレー（にじみ・残像を軽減） |
| 面 | `#26292B` | 一段明るい面で要素を分離 |
| 文字 | `#DAD6CF` | 暖色オフホワイト（純白を避ける） |
| アクセント | `#4FA98F`（ティール） | 主要操作。低彩度でブルーライト控えめ |
| 実行系 | `#D2A24C`（アンバー） | 「一括実行」の1点のみ |

字体は丸ゴシック（`Yu Gothic UI`）で仮名・漢字の可読性を確保。①→④の番号付き一方向導線で
視線移動を最小化しています。画面イメージのプレビュー（HTML）は別途共有。

画面構成（ワークシート・タブで切り替え）:

```
[コンソール] [CTSQ-DATA] [ACROS-DATA] [WORK-HISTORY] [メール作成]

コンソール
  ① 受付            CASE_ID:[____]  (① 受付データ取得)
  ② 情報確認        (作業履歴を取得) (マッピング反映)
  ③ 掲示板          (掲示板を開く)
  ④ CE指示          (CE 指示メールを作成)
  ▼ まとめて実行    (① → ④ 一括実行)
```

- 各データシート左上に「▲ コンソールへ戻る」ボタンを配置。
- `BuildConsole` は再実行してもコンソールのボタンを作り直し、他シートは「戻る」ボタンのみ
  張り替えるため、ユーザーが作った既存ボタンは壊しません。

> 補足: これは Excel のワークシート・タブを使ったコンソールです。UserForm の MultiPage による
> ポップアップ型タブ UI が良ければ、そちらにも作り替え可能です（要相談）。

## 導入方法

1. Excel で VBE（`Alt` + `F11`）を開く。
2. 対象ブックを選択し、`ファイル` → `ファイルのインポート` から
   `CaseDataRetrieval.bas` `CEInstruction.bas` `Mapping.bas` `Console.bas` を読み込む。
...
7. `Console.BuildConsole` を 1 回実行し、「コンソール」画面とタブ・ナビを生成する。
8. Outlook がインストール済みであること（`CreateCEInstructionMail` が使用）。
