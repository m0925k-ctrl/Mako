...
「読みやすい要約」を付ける方法は2つ。**併用可**。

### 案A：閲覧時にオンデマンド生成（実装済み）
- Web版ダッシュボード（`web/index.html`）に実装済み。
- 「🧠 このカルテをAI要約」ボタンで、閲覧者の Claude アカウントを使い
  その場で「次の担当者向け申し送り」を生成（現状/繰り返す事象/残務/推奨アクション）。
- **メリット**：バックエンド不要。すぐ使える。常に最新の履歴で要約。
- **注意**：閲覧者に Claude 利用環境が必要。要約は保存されず都度生成。

### 案B：登録時に生成して SharePoint に保存
- Power Automate から AI（Azure OpenAI / Claude API / AI Builder）を呼び、
  `WorkReports.AI要約` 列に保存。
- **メリット**：誰が見ても同じ要約が残る。オフライン閲覧・検索対象にできる。
- **注意**：API 契約・キー管理・コストが必要。

> 推奨：まず案A（今すぐ）。定着したら案Bで「保存される要約」も追加。
